/**
 * Hearts-specific score entry modal.
 * Includes bonus buttons for Queen of Spades and Shoot the Moon.
 */
...
interface HeartsScoreModalProps {
  visible: boolean;
  onClose: () => void;
  onSave: (
  ...
  players: Player[];
  editRoundScores?: Record<string, number>;
  editBonusType?: HeartsBonusType;
  editBonusPlayerId?: string;
  editRoundIndex?: number | null;
}

export function HeartsScoreModal({
  visible,
  onClose,
  onSave,
  players,
  ...
  const [originalScores, setOriginalScores] = useState<Record<string, string>>(
    {}
  );
  ...
  function handleMoonToggle(playerId: string) {
    if (shootMoonPlayerId === playerId) {
      ...
      const moonScores: Record<string, string> = {};
      players.forEach((p) => {
        if (p.id === playerId) {
          moonScores[p.id] = "0";
        } else {
